%%% @doc Plugin registrar - registers MeshView with hecate-daemon.
%%%
...
%%% @end
-module(app_meshviewd_plugin_registrar).
...
init([]) ->
    ...
    case attempt_registration() of
        ok ->
            logger:info("[app_meshviewd_plugin_registrar] Registered with hecate-daemon");
        {error, Reason} ->
            logger:warning("[app_meshviewd_plugin_registrar] Could not register "
...
attempt_registration() ->
    ...
    case filelib:is_file(DaemonSocket) of
        ...
        true ->
            logger:info("[app_meshviewd_plugin_registrar] Found daemon socket at ~s, "
